...
class TelemetryService {
    ...
    dispatch(req, res) {
        ...
        if (this.config.localStorageEnabled === 'true' || this.config.telemetryProxyEnabled === 'true') {
            ...
            this.sendError(res, { id: 'api.observability ', params: { err: 'Configuration error' } });
        }
    }
    health(req, res) {
    ...
    getRequestCallBack(req, res) {
        return (err, data) => {
            if (err) {
                ...
                this.sendError(res, { id: 'api.observability', params: { err: err } });
            }
            else {
                this.sendSuccess(res, { id: 'api.observability' });
            }
        }
    }
    ...
    telemetryServiceHealth(req, res) {
        this.sendSuccess(res, { id: 'observability.api.service.health', responseCode: 'Ok' });
    }
}
